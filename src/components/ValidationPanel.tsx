...
const severityStyle = {
  critical: 'border-red-500/45 bg-red-500/10 text-red-100',
  warning: 'border-amber-500/45 bg-amber-500/10 text-amber-100',
  info: 'border-sky-500/45 bg-sky-500/10 text-sky-100'
};
    ...
    <section className="rounded-lg border border-slate-800 bg-slate-900/78 p-4">
      ...
      >
        ...
        <div className="rounded-md border border-slate-800 bg-slate-950 p-2">
          ...
          <div className="mt-1 font-semibold text-white">{totals.weightKg.toFixed(1)}kg</div>
        </div>
        <div className="rounded-md border border-slate-800 bg-slate-950 p-2">
          <div className="text-slate-500">Power</div>
          <div className="mt-1 font-semibold text-white">{totals.powerW}W</div>
        </div>
        <div className="rounded-md border border-slate-800 bg-slate-950 p-2">
          <div className="text-slate-500">Used U</div>
          <div className="mt-1 font-semibold text-white">{totals.occupiedU}</div>
        </div>
      </div>

      {issues.length === 0 ? (
        <div className="flex items-start gap-3 rounded-md border border-emerald-500/35 bg-emerald-500/10 p-3 text-sm text-emerald-100">
          ...
          {issues.map((issue) => (
            <button
              ...
              className={`w-full rounded-md border p-3 text-left text-sm transition hover:brightness-110 ${
                selectedIssueId === issue.id ? 'ring-2 ring-cyan-300/70' : ''
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="mt-2 rounded border border-white/10 bg-black/15 px-2 py-1.5 text-[11px] leading-4 opacity-95">
