...
import { getDepthSummary } from '../utils/rackMath';
...
export function DepthCompatibilityPanel() {
  ...
  const issueDevices = layout.devices
    .filter((d) => {
      if (d.sizeU === 0) return false;
      const usableDepth = summary.usableDepthMm;
      const railMin = layout.railMinDepthMm ?? 0;
      const railMax = layout.railMaxDepthMm ?? layout.rackDepthMm;
      return d.depthMm > usableDepth || d.depthMm < railMin || d.depthMm > railMax;
    })
    .map((d) => ({
      device: d,
      tooDeep: d.depthMm > summary.usableDepthMm,
      tooShallow: d.depthMm < (layout.railMinDepthMm ?? 0),
      tooDeepForRails: d.depthMm > (layout.railMaxDepthMm ?? layout.rackDepthMm),
    }));
      ...
      <div className="space-y-3">
        ...
        {issueDevices.length > 0 ? (
          <div className="space-y-1.5">
            ...
            {issueDevices.map(({ device, tooDeep, tooShallow, tooDeepForRails }) => (
              <button
                key={device.id}
                ...
                type="button"
              >
                <Minimize size={10} className={tooDeep ? 'text-red-400' : tooDeepForRails ? 'text-amber-400' : 'text-sky-400'} />
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px]" style={{ color: 'var(--theme-text-muted)' }}>
                    {device.depthMm}mm
                    {tooDeep && ` > usable ${summary.usableDepthMm}mm`}
                    {tooShallow && ` < rail min ${layout.railMinDepthMm}mm`}
                    {tooDeepForRails && ` > rail max ${layout.railMaxDepthMm}mm`}
                  </div>
                </div>
              </button>
            ))}
