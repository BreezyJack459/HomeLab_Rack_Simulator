...
const lensMeta = {
  overview: {
    ...
    description:
      "Cross-check rack health, drift and current risk in one glance.",
  },
  issues: {
    label: "Issues",
    icon: <AlertTriangle size={14} />,
    description:
      "Work the validation queue from highest severity to easiest next fix.",
  },
  serviceability: {
    label: "Serviceability",
    icon: <Cable size={14} />,
    description:
      "Focus on pull-out blockers, strain risks and maintenance access.",
  },
  documentation: {
    label: "Documentation",
    icon: <ClipboardList size={14} />,
    description:
      "Tighten labels, evidence and cable annotations before drift accumulates.",
  },
  thermal: {
    label: "Thermal",
    icon: <Flame size={14} />,
    description:
      "Review heat pressure, power headroom and environmental telemetry.",
  },
  domains: {
    label: "Domains",
    icon: <ShieldAlert size={14} />,
    description:
      "Check redundancy, assignment gaps and domain-level blast radius.",
  },
} as const;
...
export function AuditWorkbench({
  ...
  const occupiedPct =
    layout.heightU > 0
      ? Math.round((totals.occupiedU / layout.heightU) * 100)
      : 0;
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-950/50">
          ...
          {topIssues.length === 0 ? (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm text-emerald-700 dark:text-emerald-300">
              Audit queue is clear. Use the lens chips above to inspect thermal,
              documentation or domain health.
            </div>
          ) : (
